...
def simple_epoch_single_chain(n_epoch, optimizer, seq_modules, input_states, seed):
    """Routine that trains the network for n_epoch times calling local_energies_SYK as cost function"""
    ...
    for i in range(n_epoch):
        ...
        local_energies = local_energies_SYK(seq_modules, input_states, seed)
    ...
    print("After ", n_epoch, " epochs,  E_ground =", mc_energy)
    #print("   local energies :", local_energies)
    return mc_energy, local_energies
...
def single_chain(batch_chain, tau, Net):
    """dim = 0 of batch_chain its the length of the markov-chain that we want to optimize. tau is the number of single steps from one point sampled and the other
    (ideally greater than the self-correlation time). """
    batch_size = batch_chain.shape[0]
    L = batch_chain.shape[1]
    first_state = batch_chain[0, :]
    first_state = first_state.unsqueeze(dim = 0)
    
    for i in range(batch_size):
        for _ in range(tau):
          first_state = Markov_step(first_state, Net)[0]
        batch_chain[i, :] = first_state
